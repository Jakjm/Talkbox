 ...
 * Each TalkBox device has a number of audio buttons, followed by a number of
 * swap buttons. An audio button plays an audio file when pressed. A swap button
 * associates the audio buttons with a new set of audio files when pressed. Swap
 * button 1 selects audio set 1, Swap button 2 selects audio set 2 etc. If there
 * are not enough swap buttons for each audio set, then the last swap button
 * cycles through all remaining audio sets.
 * 
 * @author bil
 *
 */
